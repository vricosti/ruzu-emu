...
fn compile_vulkan_present_shaders(manifest_dir: &std::path::Path) {
    ...
    let shaders = [
        ...
        ("BLIT_DEPTH_FRAG_SPV", "blit_depth.frag"),
        ...
        ("CONVERT_MSAA_TO_NON_MSAA_DEPTH_FRAG_SPV", "convert_msaa_to_non_msaa_depth.frag"),
        ("CONVERT_MSAA_TO_NON_MSAA_DEPTH_STENCIL_FRAG_SPV", "convert_msaa_to_non_msaa_depth_stencil.frag"),
        ("CONVERT_NON_MSAA_TO_MSAA_FRAG_SPV", "convert_non_msaa_to_msaa.frag"),
        ("CONVERT_NON_MSAA_TO_MSAA_DEPTH_FRAG_SPV", "convert_non_msaa_to_msaa_depth.frag"),
        ("CONVERT_NON_MSAA_TO_MSAA_DEPTH_STENCIL_FRAG_SPV", "convert_non_msaa_to_msaa_depth_stencil.frag"),
    ...
    // (upstream `SHADER_TYPE_VARIANTS` in host_shaders/CMakeLists.txt).
    #[rustfmt::skip]
    let shader_type_variants = [
        ("CONVERT_MSAA_TO_NON_MSAA_SINT_FRAG_SPV", "convert_msaa_to_non_msaa.frag", "sint", "isampler2DMS", "ivec4"),
        ("CONVERT_MSAA_TO_NON_MSAA_UINT_FRAG_SPV", "convert_msaa_to_non_msaa.frag", "uint", "usampler2DMS", "uvec4"),
        ("CONVERT_NON_MSAA_TO_MSAA_SINT_FRAG_SPV", "convert_non_msaa_to_msaa.frag", "sint", "isampler2D", "ivec4"),
        ("CONVERT_NON_MSAA_TO_MSAA_UINT_FRAG_SPV", "convert_non_msaa_to_msaa.frag", "uint", "usampler2D", "uvec4"),
    ];
    ...
    let jobs = shaders
        .iter()
        .map(|(name, filename)| (*name, *filename, filename.to_string(), Vec::new()))
        .chain(shader_type_variants.iter().map(
            |(name, filename, suffix, sampler_type, texel_type)| {
                let stem = filename.rsplit_once('.').map_or(*filename, |(stem, _)| stem);
                (
                    *name,
                    *filename,
                    format!("{stem}_{suffix}.frag"),
                    vec![
                        format!("-DSAMPLER_TYPE={sampler_type}"),
                        format!("-DTEXEL_TYPE={texel_type}"),
                    ],
                )
            },
        ));
    for (name, filename, output_stem, defines) in jobs {
        let source = shader_dir.join(filename);
        println!("cargo:rerun-if-changed={}", source.display());
        let spv_path = out_dir.join(format!("{output_stem}.spv"));
