...
mod tests {
    ...
    fn enable_setting_controls_samples_and_wheel_history() {
        const CHILD: &str = "RUZU_TEST_DEBUG_MOUSE_SETTING";
        if std::env::var_os(CHILD).is_none() {
            let status = std::process::Command::new(std::env::current_exe().unwrap())
                .args(["--exact", "resources::mouse::debug_mouse::tests::enable_setting_controls_samples_and_wheel_history"])
                .env(CHILD, "1").status().unwrap();
            assert!(status.success());
            return;
        }
        let mut device = DebugMouse::new();
