...
use crate::graphics_shader_runtime::{
    buffer_cache_metadata, translate_graphics_stages_from_environments, TranslatedGraphicsShader,
    NUM_GRAPHICS_STAGES,
};
    ...
    pub fn current_graphics_shaders(
        ...
        if !self.graphics_shader_modules.contains_key(&key) {
            ...
            let translated = catch_shader_exception(|| {
                translate_graphics_stages_from_environments(
                    &self.host_info,
                    &key.fixed_state,
                    &key.unique_hashes,
                    key.hash_value(),
